...
import utils
from utils import parse, guidance, latents, vis, attn, torch_device
...
def generate_single_object_with_box(
    ...
    **kwargs,
    ...
    # Generate the object using GLIGEN.
    if verbose:
        print("word_token_index:", word_token_index)
    (
        latents,
        single_object_images, # the generated image of the object
        saved_attns, # the saved_cross-attention maps (used for alignment)
        single_object_pil_images_box_ann, # the visualized bounding box annotations
        latents_all, # the final latent representation of the object
    ...
    # Visualize and refine object mask with SAM.
    if visualize:
    ...
    return (
        latents_all, # the final latent representation of the object
        mask_selected_tensor, # the refined mask from SAM
        saved_attns, # the saved_cross-attention maps (used for alignment)
        single_object_pil_image_box_ann, # the visualized bounding box annotations
    )
...
def run(
    ...
    # 1. Preprocess the "spec" dictionary.
    if (len(spec["add_objects"]) + len(spec["move_objects"]) + len(spec["change_objects"])) > 0:
        add_spec = {}
        ...
        # 2. Construct a new "spec" for added objects.
        ...
        # 3. Ensure that the SO box is centered.
        if so_center_box:
            so_prompt_phrase_word_box_list = [
                (
                    prompt,
        ...
        with torch.autocast("cuda", enabled=use_autocast):
            ...
            # 4. Generate the initial latents.
            generator = torch.manual_seed(bg_seed + 87)
            initial_L = get_scaled_latents(
                51, unet.config.in_channels, 512, 512, generator, dtype, scheduler
            ).unsqueeze(1)

            # 5. Blend the background and foreground latents.
            ...
            # 6. Generate the initial background and foreground latents.
            ...
            # 8. Use masked latent generation for objects.
            if use_ref_ca or frozen_steps > 0:
                (latents_all_list, mask_tensor_list, saved_attns_list, so_img_list) = get_masked_latents_all_list(
                    so_prompt_phrase_word_box_list,
                    input_latents_list,
            ...
            # 9. Compose latents with alignments.
            ...
            # 10. Apply reference cross-attention adjustments.
            if use_ref_ca:
            ...
            # 11. Create the overall prompts and bounding boxes.
            ...
            # 12. Generate word token indices and encoding.
            (overall_object_positions, overall_word_token_indices, overall_prompt) = guidance.get_phrase_indices(
                tokenizer=tokenizer,
