...
def get_phrase_indices(
    ...
    for obj_ind, obj in enumerate(phrases):
        # 2. Tokenize the individual phrase.
        ...
        # 3. Handle attention word token indexing.
...
# pipelines.py
def add_ref_ca_loss_per_attn_map_to_lossv2(
    loss,
    saved_attn,
    object_number,
    bboxes,
    ...
    guidance_attn_keys,
    ref_ca_saved_attns,
    ref_ca_last_token_only,
    ref_ca_word_token_only,
    word_token_indices,
    index,
    loss_weight,
    eps=1e-5,
    ...
    for obj_idx in range(object_number):
        ...
        assert len(obj_boxes) == len(
            obj_ref_ca_saved_attns
        ), f"obj_boxes: {len(obj_boxes)}, obj_ref_ca_saved_attns: {len(obj_ref_ca_saved_attns)}"
        ...
        for obj_box, obj_ref_ca_saved_attn in zip(obj_boxes, obj_ref_ca_saved_attns):
            ...
            for attn_key in guidance_attn_keys:
                ...
                if ref_ca_word_token_only:
                    ...
                    print(
                        f"Applying attention transfer from one attention to all attention maps in object positions {object_positions[obj_idx]}, which is likely to be incorrect"
                    )
                ...
                for obj_position in object_positions_to_iterate:
                    ...
                    ca_map_obj_masked_normalized = ca_map_obj_masked / (
                        ca_map_obj_masked.sum(dim=-1, keepdim=True) + eps
                    )
                    obj_ref_ca_map_masked = obj_ref_ca_map * obj_mask
                    obj_ref_ca_map_masked_normalized = obj_ref_ca_map_masked / (
                        obj_ref_ca_map_masked.sum(dim=-1, keepdim=True) + eps
                    )
                    ...
                    activation_value = (
                        torch.abs(
                            ca_map_obj_masked_normalized
                            - obj_ref_ca_map_masked_normalized
                        )
                    ).sum(dim=-1)
        ...
        loss += (
            loss_weight * obj_loss / (len(obj_boxes) * len(object_positions_to_iterate))
        )

        if verbose:
            print(
                f"reference cross-attention obj_loss: unweighted {obj_loss.item() / (len(obj_boxes) * len(object_positions[obj_idx])):.3f}, weighted {loss_weight * obj_loss.item() / (len(obj_boxes) * len(object_positions[obj_idx])):.3f}"
            )
...
def compute_ca_lossv3(
    ...
    ref_ca_word_token_only=False,
    word_token_indices=None,
    index=None,
    ref_ca_loss_weight=1.0,
    verbose=False,
    **kwargs,
):
    """
    ...
    if ref_ca_saved_attns is not None:
        ...
        ref_loss = add_ref_ca_loss_per_attn_map_to_lossv2(
            ...
            ref_ca_last_token_only=ref_ca_last_token_only,
            ref_ca_word_token_only=ref_ca_word_token_only,
            word_token_indices=word_token_indices,
            verbose=verbose,
            index=index,
            loss_weight=ref_ca_loss_weight,
        )
        ...
        if verbose:
            print(
                f"loss {loss.item():.3f}, reference attention loss (weighted) {ref_loss.item() / (object_number * num_attn):.3f}"
            )
